...
class BountySystem {
    ...
    async connectWallet() {
        try {
            ...
            if (chainId.chainId !== 11155111) { // Sepolia chain ID is 11155111
                alert('Please connect to the Sepolia Ethereum Testnet');
            }
            return accounts[0];
        } catch (error) {
    ...
    async deployBountyContract(bountyHunterAddress, bountyAmount) {
        try {
            ...
            alert('Error deploying contract. Please try again.');
            throw error;
        }
    }

    // Other contract interaction methods remain unchanged...
...
// Example usage on window load
window.addEventListener('load', async () => {
    const bountySystem = new BountySystem();
    await bountySystem.checkWalletConnection();
});
